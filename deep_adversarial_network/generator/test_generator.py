...
class test_Generator2():
    ...
    def make_generator_network(self, mask, reuse=False, isTrain=True):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            deconv3 = tf.layers.conv2d_transpose(inputs=conv3_bn, filters=64, kernel_size=(3, 3), padding='same',
                                                 activation=tf.nn.relu)
            deconv3_bn = tf.layers.batch_normalization(deconv3)

            deconv2 = tf.layers.conv2d_transpose(inputs=deconv3_bn
...
class test_Generator1():
    ...
    def make_generator_network(self, mask, reuse=False, isTrain=True):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            conv1 = tf.layers.conv2d(inputs=input, filters=64, kernel_size=(5, 5), padding='valid', strides=2,
