...
class test_Generator1():
    ...
    def make_generator_network(self, mask, reuse=False, isTrain=True):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            fc5 = tf.layers.dense(fc4, units=16*16*64)

            fc5_reshape = tf.reshape(fc5, shape=[-1, 16 , 16 , 64])
            ...
            deconv2 = tf.layers.conv2d_transpose(inputs=deconv3_bn, filters=32, kernel_size=(3, 3), padding='same',
                                                 activation=tf.nn.relu)
