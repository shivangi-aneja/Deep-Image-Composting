...
class TOY_DATA(BaseDataset):
    ...
    def _load(self, dirpath):
        ...
        val_data = torch.stack([torch.Tensor(i) for i in image_tuple[1:2]])
...
class BIG_DATA(BaseDataset):
    ...
    def _load(self, dirpath):
        # Training images
        image_tuple = np.load(dirpath + '/big_data.npy')

        train_data = torch.stack([torch.Tensor(i) for i in image_tuple[0:2]])
        val_data = torch.stack([torch.Tensor(i) for i in image_tuple[2:]])
